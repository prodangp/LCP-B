...
def load_trace(window_size=9):
    ...
    for protein in data[permutation][:N]:
        ...
        for aa in range(len(protein["aminoacids"])):
            sequence = np.array(x[aa: window_size + aa]).astype(np.float32)
            label = y[aa: window_size + aa]
...
class SeqData:
    def __init__(self, window_size):
        train_x, train_y = list(zip(*load_trace(window_size=window_size)))
...
class SeqModel:
    def __init__(self, model_type, model_size, epochs, window_size=1, opt='Adam', learning_rate = 0.01, activation='sigmoid'):
        ...
        if not os.path.isfile('results/seq2/exp/seq_len'):
        ...
        for e in range(epochs):
            if (verbose and e % log_period == 0):
                ...
                test_acc, test_loss = self.sess.run([self.accuracy, self.loss],
                                                    {self.x: np.transpose(gesture_data.test_x, (0, 1, 2)),
                                                     self.target_y: gesture_data.test_y})
                valid_acc, valid_loss = self.sess.run([self.accuracy, self.loss],
                                                      {self.x: np.transpose(gesture_data.valid_x, (0, 1, 2)),
            ...
            if (verbose and e % log_period == 0):
                print(
                    "Epochs {:03d}, train loss: {:0.2f}, train acc: {:0.2f}, valid loss: {:0.2f}, valid acc: {:0.2f}, test loss: {:0.2f}, test acc: {:0.2f}".format(
                        e,
                        np.mean(losses), (np.mean(accs) - (zf / (1 + zf))) * (1 + zf),
                        valid_loss, (valid_acc - (zf_v / (1 + zf_v))) * (1 + zf_v),
                        test_loss, (test_acc - (zf_t / (1 + zf_t))) * (1 + zf_t)
                ...
                with open('./results/seq2/' + str(ts) + '_' + self.result_file + '_epochs.csv', "a+") as f:
                    f.write(
                        "Epochs {:03d}, train loss: {:0.2f}, train acc: {:0.2f}, valid loss: {:0.2f}, valid acc: {:0.2f}, test loss: {:0.2f}, test acc: {:0.2f}\n".format(
                            e,
                            np.mean(losses), (np.mean(accs) - (zf / (1 + zf))) * (1 + zf),
                            valid_loss, (valid_acc - (zf_v / (1 + zf_v))) * (1 + zf_v),
                            test_loss, (test_acc - (zf_t / (1 + zf_t))) * (1 + zf_t)
...
if __name__ == "__main__":
    ...
    seq_data = SeqData(window_size=args.window)
    model = SeqModel(model_type=args.model, model_size=args.size, opt=args.opt, epochs=args.epochs,
                     learning_rate=args.lr, activation=args.activation, window_size=args.window)
